...
seqs_fasta <- readDNAStringSet("C:/Users/vhogan/Documents/Flu/ARIA_fastas/A10-10599-6-PCR_NA.fa")
  ...
  if(type == "H1N1" & gene == "NA"){
    flag_t <- df %>% 
      mutate(flu_antiviral = case_when(df$`223` != "I" & df$`247` != "S" & df$`275` != "H" 
                                                            ~ paste0("NA:I223", df$`223`, "NA:S247", df$`247`, "NA:H275", df$`275`),
...
flag <- flag %>%
  dplyr::rename(SampleID = name) #error when package isn't specified
...
###SAVE FILE##########
write.csv(output, "/Users/vhogan/Documents/Flu/A10-10599-6-PCR_NA.csv", row.names = FALSE, na = "")
